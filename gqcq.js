...
class UserInfo {
        ...
        try {
            ...
            if (typeof result !== 'undefined' && 'errorCode' in result) {
                if (result.errorCode == 200) {
                    ...
                    let username = result.data.mobile
    ...
    async post_topic() {//userinfo
        try {
            let options = {
                ...
                headers: this.headersOne,
                body: `postId=&postType=2&channelInfoId=116&columnId=&postContent=[{"text":"${this.getText()}"}]&coverImg=https://pic-gsp.gacmotor.com/app/712e2529-7b85-4d70-8c71-22b994b445b5.jpg&publishedTime=&contentWords=${this.getText()}&contentImgNums=1&lng=&lat=&address=&cityId=`
            }
            //console.log(options);
            let result = await httpRequest(options);
            //console.log(result);
    ...
    async add_comment(topic_id) {//userinfo
        // console.log(this.getCommentText())
        let txt = await yiyan()
        try {
            let options = {
                url: `https://gsp.gacmotor.com/gw/app/community/api/comment/add`,
                headers: this.headersOne,
                body: `commentType=0&postId=${topic_id}&commentContent=${txt}&commentId=0&commentatorId=NDc3ODY1MA==&isReplyComment=1`
            }
            console.log(options);
            let result = await httpRequest(options);
            //console.log(result);
