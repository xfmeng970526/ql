...
cookies = os.getenv('android_dyjsb')  # 变量名

ua = 'com.ss.android.ugc.aweme.lite/250101 (Linux; U; Android 10; zh_CN_#Hans; LIO-AN00; Build/HUAWEILIO-AN00;tt-ok/3.12.13.1)'
debug_mode = 0
remain_water_num = 50  # 剩余水滴数
exec_orchard = True  # 是否执行果园，默认True执行

# 任务执行时间
task_schedule = {
	'origin_task': '00:00:00-23:59:00',
	'orchard_task': '000:00:00-23:59:00'
}
...
class User:
	...
	def __init__(self, cookie1):
		...
		try:
			temp = self.params
			if temp.rfind('mode=done&') != -1:
				self.clear_mode_params = re.findall('mode=done&(.*?)$', self.params)[0]
				self.exec_shopping = True
			else:
				self.clear_mode_params = temp
		except Exception:
	...
	def find_params(self):
		...
		try:
			if task_id_dict.get('逛街赚钱') in task_id_arr:
				...
				if shopping_cur_count == shopping_max_count:
					...
					self.shopping_completed = False

			if task_id_dict.get('签到') in task_id_arr:
				sign_info = task_list[task_id_arr.index(task_id_dict.get('签到'))]
			else:
				sign_info = task_list[task_id_arr.index(task_id_dict.get('新人签到'))]
	...
	def get_info(self):
		...
		if response.get('err_no') == 0:
			...
			cash_amount = response.get('data').get('income_data').get('amount2')
			total_amount = cash_amount * 100 + amount
			cash_amount = ('%.2f' % (cash_amount / 100))
			total_amount = ('%.2f' % (total_amount / 10000))
			print(f'【查询信息】今日金币{amount},现金{cash_amount},总额{total_amount}')
			if debug_mode == 1:
	...
	def task(self, task_key):
		...
		if task_key == 'done/sign_in':
			...
			if res.get('err_no') == 0:
				...
				amount = res.get('data').get('amount')
				print(f"【看广告赚金币】第1个视频获得{amount}金币")
				self.amount += amount
				return 1
			...
			if res.get('err_no') == 0:
				...
				return 0
		elif task_key == 'walk/step_submit':
			payload = json.dumps({
				"step": 300,
			...
			if res.get('err_no') == 0:
				return 1
			# else:
			# 	print(f'【上传步数】没有返回金币数量')
			# 	return 0
			else:
				print(f'【上传步数】' + res.get('err_tips'))
				return 0
		...
		# ======================================== get方法 ====================================
			...
			try:
				...
				# print(res)
				if res.get('err_no') == 0:
					if 'data' in res and 'today_step' in res.get('data'):
						today_step = res.get('data').get('today_step')
						return today_step
					else:
						print(f'【获取步数】没有返回步数')
						return -1
				else:
					print(f'【获取步数】' + res.get('err_tips'))
					return -1
			except Exception:
				print('【walk/page】函数异常')
		...
		try:
			for count in range(water_count):
				...
				if res.get('status_code') == 0:
					...
					if self.shake_tree and (shake_tree_state == 2 or shake_tree_state == 3):
	...
	def run_test(self):
		...
		for token in todo_token_list:
			...
			for key in keys:
				...
				if self.game_orchard_team_gain_water(water_index=key, water_type=water_type,
													 friend_token=token) == 0:
					break
			time.sleep(random.uniform(5, 10))

		three_gift_dic = self.game_orchard_three_gift_list()
		print(three_gift_dic)
	...
	def origin_run(self):
		...
		if cur_time < self.treasure_box_next_time:
			...
			if self.task('done/treasure_task'):  # 开宝箱
				if self.task('done/excitation_ad_treasure_box'):  # 看宝箱广告
					self.task('done/excitation_ad/one_more#excitation_ad_treasure_box')
		# 逛街任务
		if self.shopping_completed is None or self.exec_shopping is False:
			print('【逛街赚金币】不执行')
		elif self.shopping_completed:
			print('【逛街赚金币】已完成')
		elif self.exec_shopping and self.shopping_completed is False:
			self.task('done/shopping_gold')

	def run(self):

		origin_task_time = task_schedule.get('origin_task')
		origin_task_time_start = re.findall('^(.*?)-', origin_task_time)[0]
		origin_task_time_end = re.findall('-(.*?)$', origin_task_time)[0]
		format_time = time.strftime('%H:%M:%S')

		if origin_task_time_start <= format_time <= origin_task_time_end:
			self.find_params()
			self.origin_run()
		...
		orchard_task_time = task_schedule.get('orchard_task')
		orchard_task_time_start = re.findall('^(.*?)-', orchard_task_time)[0]
		orchard_task_time_end = re.findall('-(.*?)$', orchard_task_time)[0]
		if exec_orchard and orchard_task_time_start <= format_time <= orchard_task_time_end:
			print('等待执行果园任务')
			time.sleep(random.uniform(8, 20))
			self.game_orchard_run()
		elif exec_orchard is False:
			print('设置不执行果园任务')
		else:
			print('不在果园任务执行时间')
