 ...
 * 广汽传祺
 * cron: 18 10,18,22 * * *
 * new Env('25-广汽传祺_0521');
 * 项目名称：广汽传祺
 * 2023/03/20 随机延迟 一天运行3次 建议每个脚本不超过8个号
 * 一天运行2-3次 改掉默认定时 
 * ========= 青龙--配置文件 ===========
 * # 项目名称
    ...
    async post_topic() {//userinfo
        ...
        try {
            let options = {
                ...
                headers: this.headersOne,
                body: `postId=&postType=2&channelInfoId=116&columnId=&postContent=[{"text":"${txt}"}]&coverImg=https://pic-gsp.gacmotor.com/app/712e2529-7b85-4d70-8c71-22b994b445b5.jpg&publishedTime=&contentWords=${txt}&contentImgNums=1&lng=&lat=&address=&cityId=`
            }
            //console.log(options);
            let result = await httpRequest(options);
            //console.log(result);
