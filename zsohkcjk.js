 ...
 * 下面我们来看看函数需要注意的东西吧
 */
 function chaxunkucun(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `http://lwintegral.wznbw.com/addons/leescore/index/ajaxGoodlist?appid=2`,    // 这是请求的 url 可以直接用我们抓包、精简后的URL
			headers: {
    				"Host": "lwintegral.wznbw.com"
  },
		...
		$.get(url, async (error, response, data) => {     // 这是一个 get 请求 , 如果是 post  记得把这里改了 
			try {
				...
				let result = JSON.parse(data);
								...
								console.log(` ${result.data[c].name}，当前库存：${result.data[c].stock}\n `)
...
async function Envs() {
	if (zsohkcjk_data) {
		if (zsohkcjk_data.indexOf("@") != -1) {
			zsohkcjk_data.split("@").forEach((item) => {
				zsohkcjk_dataArr.push(item);
			});
		} else {
			zsohkcjk_dataArr.push(zsohkcjk_data);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 zsohkcjk_data`)
		return;
	}
